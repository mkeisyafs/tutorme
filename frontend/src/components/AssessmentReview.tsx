...
const MultipleChoiceReviewCard = ({ review, index }: { readonly questionId: string; readonly review: MultipleChoiceQuestionReview; readonly index: number }) => (
  <article className="rounded-2xl border-2 border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
    <div className="flex items-center justify-between gap-3">
      <p className="text-sm font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Question {index + 1} · Multiple choice</p>
      {review.isCorrect
        ? <span className="inline-flex items-center gap-1.5 rounded-full bg-green-100 px-3 py-1 text-xs font-bold text-green-700 dark:bg-green-900/40 dark:text-green-300"><CheckCircle2 className="h-3.5 w-3.5" /> Correct</span>
        : <span className="inline-flex items-center gap-1.5 rounded-full bg-red-100 px-3 py-1 text-xs font-bold text-red-700 dark:bg-red-900/40 dark:text-red-300"><XCircle className="h-3.5 w-3.5" /> Incorrect</span>
      }
    </div>
    <div className="mt-4 space-y-2.5">
      {review.options.map((option) => (
        <div key={String(option.index)} className={'rounded-xl border-2 p-3.5 transition-colors ' + optionClassName(option)}>
          <div className="flex items-start justify-between gap-3">
            <p className="font-bold text-gray-900 dark:text-gray-100">{option.text}</p>
            <OptionBadge option={option} />
          </div>
          {option.explanation && (option.isSelected || option.isCorrect) && (
            <p className="mt-2 text-sm font-medium leading-relaxed text-gray-600 dark:text-gray-300">{option.explanation}</p>
          )}
        </div>
      ))}
    </div>
...
const EssayReviewCard = ({ review, learnerAnswer, index }: { readonly questionId: string; readonly review: QuestionEntry[1] & { readonly type: 'ESSAY' }; readonly learnerAnswer: QuizAnswer | undefined; readonly index: number }) => (
  <article className="rounded-2xl border-2 border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
    <div className="flex items-center justify-between gap-3">
      <p className="text-sm font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Question {index + 1} · Essay</p>
      <span className="inline-flex items-center gap-1.5 rounded-full bg-blue-100 px-3 py-1 text-xs font-bold text-blue-700 dark:bg-blue-900/40 dark:text-blue-300"><FileText className="h-3.5 w-3.5" /> {review.score}%</span>
    </div>
    <div className="mt-4 rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900/40">
      <p className="text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">Your response</p>
      <p className="mt-2 whitespace-pre-wrap text-sm font-medium leading-relaxed text-gray-700 dark:text-gray-200">{answerText(learnerAnswer)}</p>
    </div>
    <div className="mt-3 rounded-xl border border-blue-200 bg-blue-50/60 p-4 dark:border-blue-800 dark:bg-blue-950/20">
      <p className="text-xs font-bold uppercase tracking-wider text-blue-500 dark:text-blue-400">AI feedback</p>
      <p className="mt-2 whitespace-pre-wrap text-sm font-medium leading-relaxed text-gray-700 dark:text-gray-200">{review.rationale}</p>
    </div>
    {(review.strengths.length > 0 || review.improvements.length > 0) && (
      <div className="mt-3 grid gap-3 md:grid-cols-2">
        {review.strengths.length > 0 && (
          <div className="rounded-xl border border-green-200 bg-green-50/60 p-4 dark:border-green-800 dark:bg-green-950/20">
            <p className="text-xs font-bold uppercase tracking-wider text-green-600 dark:text-green-400">Strengths</p>
            <ul className="mt-2 list-disc space-y-1 pl-4 text-sm font-medium text-gray-700 dark:text-gray-200">
              {review.strengths.map((s, i) => <li key={String(i)}>{s}</li>)}
            </ul>
          </div>
        )}
        {review.improvements.length > 0 && (
          <div className="rounded-xl border border-orange-200 bg-orange-50/60 p-4 dark:border-orange-800 dark:bg-orange-950/20">
            <p className="text-xs font-bold uppercase tracking-wider text-orange-600 dark:text-orange-400">Areas to improve</p>
            <ul className="mt-2 list-disc space-y-1 pl-4 text-sm font-medium text-gray-700 dark:text-gray-200">
