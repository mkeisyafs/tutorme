...
export const WarningCard: React.FC<WarningBlock> = ({ title, items }) => {
  ...
  const isGenericTitle = !title || /^(Common pitfall|Common Pitfalls|Watch Out|Kesalahan Umum)$/i.test(title.trim());
  const displayTitle = isGenericTitle ? t('blocks.warning.defaultTitle') : title;

  const displayItems = items.map((item) => {
    if (i18n.language === 'id' && item.includes('Do not skip the explanation before attempting the practice.')) {
      return 'Jangan melewatkan penjelasan sebelum mencoba latihan.';
    }
    return item;
  });
