...
const Quiz = () => {
  ...
  const notebookQuiz = location.state as { notebookTitle?: string; notebookContent?: string } | null;
  ...
  return <div className="min-h-screen bg-gray-50 font-['Nunito',sans-serif] text-gray-800 transition-colors duration-300 dark:bg-gray-900 dark:text-gray-100">
    ...
    <div className="mx-auto max-w-5xl pb-16">
      ...
      <header className="rounded-3xl border-4 border-blue-300 bg-blue-100 p-7 shadow-[7px_7px_0_#60a5fa] dark:border-blue-700 dark:bg-blue-900/35 dark:shadow-[7px_7px_0_#1e3a8a]"><p className="flex items-center gap-2 text-sm font-extrabold uppercase tracking-wider text-blue-700 dark:text-blue-300"><Target className="h-4 w-4" /> {notebookQuiz?.notebookTitle ? 'Quiz from notebook' : 'Automatically generated lesson quiz'}</p><h1 className="mt-2 font-['Kalam',cursive] text-4xl font-bold text-blue-950 dark:text-blue-100">{notebookQuiz?.notebookTitle ?? 'What is Back-End?'}</h1><p className="mt-2 font-semibold text-blue-800 dark:text-blue-200">{notebookQuiz?.notebookContent ? 'Generated using the notes you selected. ' : ''}{questions.length} questions · {settings.quizLength === 'Random' ? 'Random length selected by your course creator' : settings.quizLength} · Complete this quiz to unlock the next lesson.</p></header>
