...
const Library = () => {
  ...
  const [category, setCategory] = useState(t('library.allTopics'));
  ...
  const loadCourses = useCallback(async () => {
    ...
    try {
      ...
      setCourses([]);
      setMyCourses([]);
      setError(getApiErrorMessage(requestError, t('library.loadLibraryError')));
    } finally {
      setIsLoading(false);
    }
  }, [t]);
  ...
  const categories = useMemo(() => [t('library.allTopics'), ...new Set(courses.map((course) => course.category).sort())], [courses, t]);
  ...
  const openPreview = async (course: LibraryCourse) => {
    ...
    try {
      ...
      setPreviewError(getApiErrorMessage(requestError, t('library.loadPreviewError')));
    } finally {
  ...
  const publishCourse = async () => {
    if (!selectedCourse) {
      setFormError(t('library.shareFormError'));
      return;
    }
    ...
    try {
      ...
      setCourseSearch('');
      setNotice(t('library.shareSuccess', { title: selectedCourse.title }));
      await loadCourses();
    } catch (requestError) {
      setFormError(getApiErrorMessage(requestError, t('library.shareError')));
    } finally {
  ...
  const reuseCourse = async (course: LibraryCourse) => {
    ...
    try {
      ...
      setNotice(t('library.addedSuccess', { title: course.title }));
      window.setTimeout(() => navigate(`/courses/${course.id}`), 650);
    } catch (requestError) {
      setError(getApiErrorMessage(requestError, t('library.addError')));
    }
  };
  ...
  return (
    <DashboardLayout>
      <div className="max-w-6xl mx-auto md:mx-0 pb-12 sm:pb-16">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 sm:gap-6 mb-8 sm:mb-10">
          <div>
            <div className="inline-flex items-center gap-1.5 sm:gap-2 rounded-full bg-purple-100 dark:bg-purple-900/40 px-3.5 py-1 sm:px-4 sm:py-1.5 text-purple-800 dark:text-purple-200 font-bold text-xs sm:text-sm mb-3 sm:mb-4 border border-purple-200 dark:border-purple-700"><Users className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> {t('library.badge')}</div>
            <h1 className="text-3xl sm:text-5xl font-['Kalam',cursive] font-bold text-gray-900 dark:text-gray-100 mb-1.5 sm:mb-2 flex items-center gap-2.5 sm:gap-4 leading-tight"><LibraryBig className="w-8 h-8 sm:w-10 sm:h-10 text-purple-500 shrink-0" /> {t('library.title')}</h1>
            <p className="text-gray-600 dark:text-gray-400 font-bold text-sm sm:text-lg">{t('library.subtitle')}</p>
          </div>
          <button onClick={() => setShowUpload(true)} className="w-full sm:w-auto bg-purple-500 hover:bg-purple-600 text-white font-bold py-2.5 sm:py-3 px-5 sm:px-6 rounded-xl border-2 border-purple-700 shadow-[0_4px_0_#6b21a8] sm:shadow-[0_5px_0_#6b21a8] hover:translate-y-0.5 hover:shadow-[0_3px_0_#6b21a8] transition-all font-['Kalam',cursive] text-lg sm:text-xl flex items-center justify-center gap-2 shrink-0"><Upload className="w-4 h-4 sm:w-5 sm:h-5" /> {t('library.shareBtn')}</button>
        </div>

        <section className="bg-purple-100 dark:bg-purple-900/35 border-3 border-purple-300 dark:border-purple-700/60 rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 shadow-[4px_4px_0_#c084fc] sm:shadow-[6px_6px_0_#c084fc] dark:shadow-[4px_4px_0_#581c87] sm:dark:shadow-[6px_6px_0_#581c87] mb-8 sm:mb-12 relative overflow-hidden">
          <div className="absolute -right-8 -top-8 text-purple-300/40 dark:text-purple-400/15 pointer-events-none"><Sparkles className="w-32 h-32 sm:w-40 sm:h-40" /></div>
          <div className="relative flex flex-col md:flex-row md:items-center gap-4 sm:gap-5 justify-between">
            <div>
              <h2 className="font-['Kalam',cursive] text-xl sm:text-3xl font-bold text-purple-950 dark:text-purple-100">{t('library.bannerTitle')}</h2>
              <p className="text-purple-800 dark:text-purple-200 font-semibold text-xs sm:text-base mt-1">{t('library.bannerSubtitle')}</p>
            </div>
            <button onClick={() => setShowUpload(true)} className="w-full md:w-auto shrink-0 bg-white/80 dark:bg-gray-800/80 text-purple-800 dark:text-purple-200 border-2 border-purple-300 dark:border-purple-600 rounded-xl px-4 py-2.5 sm:px-5 sm:py-3 font-bold text-sm sm:text-base hover:bg-white dark:hover:bg-gray-800 transition-colors flex gap-2 items-center justify-center"><FileUp className="w-4 h-4 sm:w-5 sm:h-5" /> {t('library.shareBtn')}</button>
          </div>
        </section>
        ...
        <div className="flex flex-col md:flex-row gap-3 sm:gap-4 mb-6 sm:mb-8">
          <label className="relative flex-grow">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t('library.searchPlaceholder')} className="w-full rounded-xl border-2 border-gray-300 dark:border-gray-700 bg-white/85 dark:bg-gray-800/85 py-2.5 sm:py-3 pl-10 sm:pl-12 pr-4 font-bold text-xs sm:text-base text-gray-800 dark:text-gray-100 focus:outline-none focus:border-purple-400 focus:ring-4 focus:ring-purple-200 dark:focus:ring-purple-900/50" />
          </label>
          <div className="relative md:w-56">
            <button type="button" aria-haspopup="listbox" aria-expanded={isCategoryMenuOpen} onClick={() => setIsCategoryMenuOpen((isOpen) => !isOpen)} className={`flex w-full items-center justify-between rounded-xl border-2 bg-white/85 dark:bg-gray-800/85 py-2.5 sm:py-3 pl-4 pr-3 font-bold text-xs sm:text-base text-gray-700 dark:text-gray-200 transition-colors focus:outline-none focus:ring-4 focus:ring-purple-200 dark:focus:ring-purple-900/50 ${isCategoryMenuOpen ? 'border-purple-500' : 'border-gray-300 dark:border-gray-700 hover:border-purple-300'}`}>{category}<ChevronDown className={`w-4 h-4 sm:w-5 sm:h-5 text-gray-500 transition-transform ${isCategoryMenuOpen ? 'rotate-180' : ''}`} /></button>
            {isCategoryMenuOpen && <div role="listbox" aria-label="Course topic" className="absolute right-0 z-20 mt-2 w-full overflow-hidden rounded-xl border-2 border-purple-300 dark:border-purple-700 bg-white dark:bg-gray-800 shadow-[4px_4px_0_rgba(168,85,247,.35)]">{categories.map((item) => <button key={item} type="button" role="option" aria-selected={category === item} onClick={() => { setCategory(item); setIsCategoryMenuOpen(false); }} className={`flex w-full items-center justify-between px-4 py-2.5 text-left font-bold text-xs sm:text-base transition-colors ${category === item ? 'bg-purple-500 text-white' : 'text-gray-700 dark:text-gray-200 hover:bg-purple-100 dark:hover:bg-purple-900/40'}`}>{item}{category === item && <Check className="w-4 h-4" />}</button>)}</div>}
          </div>
        </div>
        ...
        {isLoading ? <div className="py-20 grid place-items-center text-purple-700 dark:text-purple-300 font-bold"><LoaderCircle className="w-8 h-8 animate-spin mb-3" />{t('common.loading')}</div> : <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 sm:gap-7">{visibleCourses.map((course, index) => {
          const style = colorStyles[course.color];
          const actionLabel = course.isMine || course.isEnrolled ? t('library.openCourse') : t('library.reuseCourse');
          return <article key={course.id} role="button" tabIndex={0} onClick={() => void openPreview(course)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); void openPreview(course); } }} className={`${style.card} ${style.border} border-2 rounded-2xl p-6 relative flex flex-col shadow-[4px_4px_0_rgba(100,116,139,.35)] ${index % 2 ? '-rotate-1' : 'rotate-1'} hover:rotate-0 transition-transform cursor-pointer focus:outline-none focus:ring-4 focus:ring-purple-300`}><div className={`absolute -top-2 left-1/2 h-5 w-16 -translate-x-1/2 ${style.tape} ${index % 2 ? 'rotate-3' : '-rotate-3'}`} /><div className="flex items-start justify-between gap-3 mb-4"><span className={`rounded-md bg-white/55 dark:bg-black/20 px-2.5 py-1 text-xs font-extrabold uppercase tracking-wide flex items-center gap-1.5 ${style.text}`}>{getCategoryIcon(course.category)}{course.category}</span>{course.isMine && <span className="text-xs font-bold text-purple-700 dark:text-purple-300">{t('library.sharedByYou')}</span>}</div><h2 className={`font-['Kalam',cursive] text-2xl font-bold leading-tight ${style.text}`}>{course.title}</h2><p className={`mt-3 text-sm font-semibold leading-relaxed opacity-80 ${style.text} flex-grow`}>{course.description}</p><p className={`mt-5 text-sm font-bold ${style.text}`}>{t('library.createdBy', { creator: course.creator })}</p><div className={`my-4 border-y border-black/10 dark:border-white/10 py-3 flex gap-4 text-sm font-bold ${style.text}`}><span className="flex items-center gap-1"><BookOpen className="w-4 h-4" /> {t('library.lessonsCount', { count: course.lessons })}</span><span className="flex items-center gap-1"><Users className="w-4 h-4" /> {course.learners.toLocaleString()}</span></div><button onClick={(event) => { event.stopPropagation(); void openPreview(course); }} className={`mb-2 w-full rounded-xl border-2 border-transparent py-2 font-bold text-sm transition-colors hover:border-white/70 hover:bg-white/35 dark:hover:bg-black/15 ${style.text}`}>{t('library.previewChaptersBtn')}</button><button onClick={(event) => { event.stopPropagation(); void reuseCourse(course); }} className={`text-white rounded-xl border-2 py-2.5 font-bold font-['Kalam',cursive] text-lg transition-all hover:translate-y-0.5 hover:shadow-none ${style.button}`}>{actionLabel}</button></article>;
        })}</div>}

        {!isLoading && visibleCourses.length === 0 && <div className="text-center bg-white/70 dark:bg-gray-800/70 rounded-2xl border-2 border-dashed border-gray-300 dark:border-gray-600 py-16"><BookOpen className="w-10 h-10 mx-auto mb-3 text-gray-400" /><h2 className="font-['Kalam',cursive] text-2xl font-bold text-gray-700 dark:text-gray-200">{t('library.noCoursesFoundTitle')}</h2><p className="text-gray-500 dark:text-gray-400 font-semibold mt-1">{t('library.noCoursesFoundDesc')}</p></div>}
      </div>

      {showUpload && <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm" onMouseDown={() => setShowUpload(false)}><section role="dialog" aria-modal="true" aria-labelledby="share-course-title" onMouseDown={(event) => event.stopPropagation()} className="relative w-full max-w-xl max-h-[90vh] overflow-y-auto rounded-3xl border-4 border-purple-400 dark:border-purple-700 bg-purple-50 dark:bg-gray-800 p-7 md:p-9 shadow-[8px_8px_0_#a855f7]"><button type="button" aria-label="Close sharing dialog" onClick={() => setShowUpload(false)} className="absolute right-5 top-5 rounded-lg p-1 text-purple-700 dark:text-purple-300 hover:bg-purple-200 dark:hover:bg-gray-700"><X /></button><h2 id="share-course-title" className="font-['Kalam',cursive] text-3xl font-bold text-purple-950 dark:text-purple-100">{t('library.shareDialogTitle')}</h2><p className="mt-1 text-purple-800 dark:text-purple-200 font-semibold">{t('library.shareDialogDesc')}</p><div className="mt-7"><label className="block font-bold text-gray-700 dark:text-gray-200">{t('library.selectCourseLabel')}</label><div className="relative mt-1.5"><Search className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-purple-500" /><input value={courseSearch} onChange={(event) => { setCourseSearch(event.target.value); setSelectedCourseId(null); setFormError(''); }} placeholder={t('library.searchMyCoursesPlaceholder')} className="w-full rounded-xl border-2 border-purple-200 dark:border-gray-600 bg-white dark:bg-gray-900 py-3 pl-12 pr-4 font-semibold focus:outline-none focus:border-purple-500" /></div><div className="mt-2 max-h-52 overflow-y-auto rounded-xl border-2 border-purple-200 dark:border-gray-600 bg-white dark:bg-gray-900">{selectableCourses.length ? selectableCourses.map((course) => <button key={course.id} type="button" onClick={() => { setSelectedCourseId(course.id); setFormError(''); }} className={`flex w-full items-center justify-between gap-3 border-b border-purple-100 dark:border-gray-700 px-4 py-3 text-left font-bold text-gray-800 dark:text-gray-100 last:border-b-0 hover:bg-purple-50 dark:hover:bg-gray-800 ${selectedCourseId === course.id ? 'bg-purple-100 dark:bg-purple-900/30' : ''}`}><span>{course.title}</span><span className="text-xs text-purple-700 dark:text-purple-300">{course.isPublic ? t('library.alreadyShared') : course.category}</span></button>) : <p className="px-4 py-3 font-semibold text-gray-500">{t('library.noMatchingCourses')}</p>}</div>{selectedCourse && <div className="mt-4 rounded-xl border-2 border-purple-200 dark:border-purple-700 bg-purple-100/70 dark:bg-purple-900/30 p-4"><p className="font-['Kalam',cursive] text-xl font-bold text-purple-950 dark:text-purple-100">{selectedCourse.title}</p><p className="mt-1 text-sm font-semibold text-purple-800 dark:text-purple-200">{selectedCourse.category} · {t('library.lessonsCount', { count: selectedCourse.lessons })}</p></div>}{formError && <p role="alert" className="mt-3 text-sm font-bold text-red-600 dark:text-red-400">{formError}</p>}</div><button type="button" disabled={isPublishing} onClick={() => void publishCourse()} className="mt-7 w-full rounded-xl border-2 border-purple-700 bg-purple-500 py-3 text-xl font-bold font-['Kalam',cursive] text-white shadow-[0_5px_0_#6b21a8] hover:translate-y-0.5 hover:shadow-[0_3px_0_#6b21a8] transition-all disabled:cursor-wait disabled:opacity-70">{isPublishing ? t('library.publishing') : t('library.publishToLibrary')}</button></section></div>}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/45 p-2 sm:p-4 pt-16 sm:pt-4 backdrop-blur-sm" onMouseDown={() => setPreviewCourse(null)}>
          ...
          >
            ...
            <div className="mt-3 sm:mt-6 border-t-2 border-purple-200 pt-3 sm:pt-4 dark:border-purple-800 flex-1 flex flex-col min-h-0">
              <h3 className="font-['Kalam',cursive] text-lg sm:text-2xl font-bold text-purple-950 dark:text-purple-100 mb-2 shrink-0">{t('library.previewTitle')}</h3>
              ...
              {isPreviewLoading ? (
                <div className="flex min-h-36 sm:min-h-44 flex-col items-center justify-center text-center font-bold text-purple-700 dark:text-purple-300">
                  ...
                  <p className="mt-3 text-sm sm:text-base">{t('library.loadingChapters')}</p>
                </div>
              ) : previewError ? (
                ...
                <div className="flex-1 overflow-y-auto min-h-0 pr-1 sm:pr-2 space-y-3 sm:space-y-4 pb-2 custom-scrollbar">
                  {previewModules.length ? (
                    ...
                    <p className="rounded-xl border-2 border-dashed border-purple-300 bg-white/70 p-4 sm:p-5 text-center font-bold text-purple-700 dark:border-purple-800 dark:bg-gray-900/50 dark:text-purple-200">{t('library.noLessonsYet')}</p>
                  )}
                </div>
              )}
            </div>
            ...
            <div className="mt-2.5 sm:mt-4 pt-2.5 sm:pt-4 border-t-2 border-purple-200 dark:border-purple-800 shrink-0 bg-purple-50 dark:bg-gray-800">
              ...
              >
                {previewCourse.isMine || previewCourse.isEnrolled ? t('library.openCourse') : t('library.reuseCourse')}
              </button>
            </div>
          </section>
        </div>
