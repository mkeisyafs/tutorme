...
import { ArrowLeft, BookMarked, FileText, MessageSquareText, Pin, Plus, Sparkles } from 'lucide-react';
  ...
  return <div className="h-screen overflow-hidden bg-[#dbeafe] text-gray-800 dark:bg-gray-900 dark:text-gray-100 font-['Nunito',sans-serif] flex">
    ...
    <main className="flex-1 overflow-y-auto p-5 md:p-10" style={{ backgroundImage: 'radial-gradient(circle at 15% 20%, rgba(167,139,250,.28), transparent 24rem), radial-gradient(circle at 90% 85%, rgba(96,165,250,.30), transparent 28rem), linear-gradient(#c7d2fe 1px, transparent 1px), linear-gradient(90deg, #c7d2fe 1px, transparent 1px)', backgroundSize: 'auto, auto, 26px 26px, 26px 26px' }}>
      {activeNotebook ? <div className="relative mx-auto max-w-4xl min-h-[calc(100vh-5rem)] rounded-[2rem] border-2 border-yellow-300 bg-[#fffdf5] p-7 md:p-12 shadow-[12px_12px_0px_rgba(76,29,149,.22)] dark:border-yellow-800 dark:bg-gray-800 overflow-hidden">
        ...
        <section className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-2xl border-2 border-yellow-200 bg-yellow-50/70 p-5 dark:border-yellow-800 dark:bg-yellow-900/20"><div><h3 className="mb-1 flex items-center gap-2 font-bold text-yellow-900 dark:text-yellow-200"><Pin className="w-4 h-4" /> Summarize document</h3><p className="text-sm leading-relaxed text-yellow-800 dark:text-yellow-300">Replace this notebook with a shorter summary of its important points.</p></div><button type="button" onClick={summarizeDocument} disabled={!body.trim()} className="flex shrink-0 items-center justify-center gap-2 rounded-xl bg-yellow-400 px-4 py-3 font-bold text-yellow-950 shadow-[3px_3px_0px_rgba(202,138,4,.55)] transition hover:bg-yellow-500 disabled:cursor-not-allowed disabled:opacity-50"><Sparkles className="w-5 h-5" /> Summary</button></section>
