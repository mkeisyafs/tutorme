...
const Roadmap = () => {
  ...
  const handlePublish = async () => {
    ...
    setActionError('');
    await startPublish(draftId, draft.courseTitle);
  };
    ...
    <div className="relative flex h-screen w-full overflow-hidden bg-white font-['Nunito',sans-serif] text-gray-800 transition-colors duration-300 dark:bg-gray-900 dark:text-gray-100">
      ...
      {isSidebarOpen && (
        <>
          ...
          <aside className="fixed inset-y-0 left-0 z-50 flex h-full w-72 flex-col justify-between overflow-y-auto border-r-2 border-dashed border-gray-300 bg-white/95 p-6 shadow-2xl transition-all duration-300 dark:border-gray-700 dark:bg-gray-800/95 lg:relative lg:z-20 lg:shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
      ...
      <main className="relative flex h-full flex-1 flex-col overflow-y-auto bg-gray-50 dark:bg-gray-900">
        <div className="relative z-10 mx-auto flex w-full max-w-4xl flex-1 flex-col px-4 sm:px-8 py-6 sm:py-10">
          ...
          <div className="mb-6 sm:mb-8 flex items-start gap-4 rounded-2xl border-4 border-purple-300 bg-purple-100 p-4 sm:p-5 shadow-[4px_4px_0px_0px_rgba(168,85,247,1)] rotate-1 transform dark:border-purple-700/50 dark:bg-purple-900/40 dark:shadow-[4px_4px_0px_0px_rgba(126,34,206,0.8)] sm:items-center">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm font-bold text-purple-800 dark:text-purple-300">This outline is a draft only. Use the editor assistant to change modules and lessons; it enters your course database only after you click <strong className="text-purple-900 dark:text-purple-200">Start Learning</strong>.</p>
            </div>
          </div>
          ...
          <div className="relative mb-6 sm:mb-8 rounded-3xl border-4 border-yellow-200 bg-yellow-50 p-5 sm:p-8 shadow-[8px_8px_0px_0px_rgba(253,224,71,1)] dark:border-yellow-700/50 dark:bg-yellow-900/20 dark:shadow-[8px_8px_0px_0px_rgba(161,98,7,0.8)]">
            ...
            {isEditingDetails ? (
              ...
              <>
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <h2 className="mb-2 sm:mb-4 font-['Kalam',cursive] text-2xl sm:text-4xl font-bold text-gray-900 dark:text-gray-100">{draft.courseTitle}</h2>
                    <p className="max-w-2xl text-sm sm:text-lg font-bold text-gray-600 dark:text-gray-300">{draft.courseDescription || `A personalized learning path for ${draft.topic}.`}</p>
                  </div>
            ...
            {draft.modules.length === 0 ? (
              ...
              return (
                <div key={module.id} className="overflow-hidden rounded-2xl border-4 border-gray-300 bg-white shadow-[4px_4px_0px_0px_rgba(156,163,175,1)] transition-all hover:border-blue-400 dark:border-gray-600 dark:bg-gray-800 dark:shadow-[4px_4px_0px_0px_rgba(55,65,81,0.8)] dark:hover:border-blue-500">
                  <button type="button" onClick={() => setExpandedModule(isExpanded ? null : module.id)} className={`flex w-full items-center gap-3 sm:gap-5 p-3.5 sm:p-5 text-left transition-colors hover:bg-gray-50 dark:hover:bg-gray-700 ${isExpanded ? 'bg-gray-50 dark:bg-gray-700/50' : ''}`} aria-expanded={isExpanded}>
                    <div className="flex h-10 w-10 sm:h-14 sm:w-14 flex-shrink-0 -rotate-6 transform items-center justify-center rounded-full border-2 border-blue-300 bg-blue-100 font-['Kalam',cursive] text-base sm:text-xl font-bold text-blue-700 shadow-[2px_2px_0px_0px_rgba(96,165,250,1)] dark:border-blue-700 dark:bg-blue-900/40 dark:text-blue-300 dark:shadow-[2px_2px_0px_0px_rgba(30,58,138,1)]">{moduleNumber}</div>
                    ...
                    <div className="flex h-9 w-9 sm:h-12 sm:w-12 items-center justify-center rounded-full border-2 border-gray-200 bg-white text-gray-600 shadow-sm transition-colors dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 shrink-0">
                      <ChevronRight className={`h-5 w-5 sm:h-6 sm:w-6 transition-transform duration-300 ${isExpanded ? 'rotate-90' : ''}`} />
                    </div>
                  </button>
                    ...
                    <div className="overflow-hidden">
                      <div className="border-t-2 border-dashed border-gray-200 bg-white px-3 sm:px-5 pb-5 pt-3 pl-8 sm:pl-14 dark:border-gray-700 dark:bg-gray-800">
                        {module.lessons.length === 0 ? (
                          <p className="font-bold text-xs sm:text-sm text-gray-500 dark:text-gray-400">No lessons in this module yet. Ask the editor to add one.</p>
                        ) : (
          ...
          <div className={`pointer-events-none fixed bottom-8 left-1/2 z-50 w-full max-w-lg -translate-x-1/2 px-4 transition-all duration-300 ${toastMessage ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
            <div className="flex items-center gap-3 rounded-2xl border-4 border-yellow-400 bg-yellow-100 px-4 sm:px-6 py-3 sm:py-4 font-['Nunito',sans-serif] text-sm sm:text-lg font-bold text-yellow-900 shadow-[4px_4px_0_rgba(234,179,8,1)] dark:border-yellow-600 dark:bg-yellow-900/80 dark:text-yellow-100">
              <Sparkles className="h-5 w-5 sm:h-6 sm:w-6 flex-shrink-0 text-yellow-600 dark:text-yellow-400" />
              {toastMessage}
            </div>
          </div>
        </div>
      ...
      {/* Right Sidebar (Outline Editor Chat) - Drawer on mobile */}
      <aside className="fixed inset-y-0 right-0 z-40 flex h-full w-80 max-w-[85vw] flex-col border-l-2 border-dashed border-gray-300 bg-purple-50/95 p-4 sm:p-6 shadow-2xl backdrop-blur-xl transition-all duration-300 dark:border-gray-700 dark:bg-gray-800/95 lg:relative lg:z-20 lg:w-80 lg:max-w-none lg:shadow-[-4px_0_24px_rgba(0,0,0,0.02)]">
        <h2 className="mb-1 sm:mb-2 flex -rotate-1 transform items-center gap-2.5 sm:gap-3 font-['Kalam',cursive] text-2xl sm:text-3xl font-bold tracking-wide text-purple-600 dark:text-purple-400">
          <MessageCircle className="h-7 w-7 sm:h-8 sm:w-8 fill-purple-500 text-purple-500" />
          Outline Editor
        </h2>
        <p className="mb-4 sm:mb-6 text-xs sm:text-sm font-bold text-purple-800 dark:text-purple-300">Assistant 1 edits this draft’s modules and lessons.</p>

        <div className="custom-scrollbar flex flex-1 flex-col gap-3 sm:gap-4 overflow-y-auto pb-4 pr-1 sm:pr-2" aria-live="polite">
          {chatMessages.map((message) => (
            <div key={message.id} className={message.role === 'assistant' ? 'mr-3 rounded-2xl sm:rounded-3xl rounded-tl-none border-2 sm:border-4 border-gray-200 bg-white p-3 sm:p-4 text-xs sm:text-md font-bold text-gray-700 shadow-[3px_3px_0px_0px_rgba(229,231,235,1)] dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300' : 'ml-3 self-end rounded-2xl sm:rounded-3xl rounded-tr-none border-2 sm:border-4 border-pink-300 bg-pink-100 p-3 sm:p-4 text-xs sm:text-md font-bold text-pink-900 shadow-[3px_3px_0px_0px_rgba(244,114,182,1)] rotate-1 transform dark:border-pink-700 dark:bg-pink-900/40 dark:text-pink-100'}>
              {message.content}
            </div>
          ))}
          {isChatting && (
            <div className="mr-3 flex items-center gap-2 rounded-2xl sm:rounded-3xl rounded-tl-none border-2 sm:border-4 border-gray-200 bg-white p-3 sm:p-4 text-xs sm:text-sm font-bold text-gray-600 shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">
              <LoaderCircle className="h-4 w-4 sm:h-5 sm:w-5 animate-spin text-purple-500" />
              Updating your outline…
            </div>
          )}
        </div>

        <form onSubmit={handleEditorChat} className="relative mt-3 sm:mt-4">
          <input
            type="text"
            value={chatInput}
        ...
        <p className="mt-2 sm:mt-3 text-center text-[0.65rem] sm:text-xs font-bold text-purple-700 dark:text-purple-300">Your outline remains a draft until Start Learning.</p>
      </aside>
    </div>
  );
};
