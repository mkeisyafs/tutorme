      ...
      <div className="max-w-4xl mx-auto md:mx-0">
        ...
        <section className="mb-16">
          <div className="bg-pink-100 dark:bg-pink-900/40 p-8 rounded-3xl border-4 border-pink-300 dark:border-pink-700/50 shadow-[8px_8px_0px_0px_rgba(244,114,182,1)] dark:shadow-[8px_8px_0px_0px_rgba(190,24,93,0.8)] transform rotate-1 relative">
            ...
            <h2 className="text-3xl font-['Kalam',cursive] font-bold text-pink-900 dark:text-pink-300 mb-4 flex items-center gap-3"><Sparkles className="w-8 h-8 fill-pink-500 text-pink-500" /> What do you want to learn today?</h2>
        ...
        <section>
          ...
          {isLoading ? <div className="py-10 flex items-center gap-3 font-bold text-gray-600 dark:text-gray-300"><LoaderCircle className="w-6 h-6 animate-spin" /> Loading recent courses…</div> : dashboard?.recentCourses.length ? <div className="grid md:grid-cols-2 gap-6">{dashboard.recentCourses.map((course, index) => {
            ...
            return <button key={course.id} onClick={() => navigate(`/courses/${course.id}`)} className={`${style.card} ${style.border} p-6 rounded-2xl border-2 shadow-[4px_4px_0px_0px_rgba(100,116,139,.35)] ${index % 2 ? '-rotate-2' : 'rotate-1'} hover:rotate-0 transition-transform cursor-pointer relative text-left`}><div className={`absolute top-0 left-1/2 w-16 h-4 ${style.tape} -translate-x-1/2 -translate-y-2`} /><h3 className={`text-2xl font-bold font-['Kalam',cursive] ${style.title} mb-2`}>{course.title}</h3><p className={`mb-4 text-sm font-bold ${style.text}`}>{course.category}</p><div className={`w-full ${style.track} rounded-full h-2 mb-2`}><div className={`${style.progress} h-full rounded-full`} style={{ width: `${progress}%` }} /></div><p className={`${style.text} font-bold text-sm`}>{progress}% Completed</p></button>;
