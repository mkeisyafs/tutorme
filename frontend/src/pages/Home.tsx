    ...
    <DashboardLayout>
      <div className="max-w-4xl mx-auto md:mx-0">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 sm:mb-12 gap-4 sm:gap-6">
          <div>
            <h1 className="text-2xl sm:text-4xl font-['Kalam',cursive] font-bold text-gray-900 dark:text-gray-100 mb-1 sm:mb-2 leading-tight">Welcome back, {displayName}! 👋</h1>
            <p className="text-gray-600 dark:text-gray-400 font-bold text-sm sm:text-lg">Ready to continue your learning journey?</p>
          </div>
          <div className="bg-orange-100 dark:bg-orange-900/40 border-2 border-orange-300 dark:border-orange-700/50 px-4 sm:px-6 py-2.5 sm:py-3 rounded-2xl shadow-[3px_3px_0px_0px_rgba(251,146,60,1)] sm:shadow-[4px_4px_0px_0px_rgba(251,146,60,1)] dark:shadow-[3px_3px_0px_0px_rgba(194,65,12,0.8)] sm:dark:shadow-[4px_4px_0px_0px_rgba(194,65,12,0.8)] transform rotate-1 sm:rotate-2 inline-flex items-center gap-2.5 sm:gap-3 self-start md:self-auto">
            ...
            <div>
              <div className="text-orange-900 dark:text-orange-300 font-bold font-['Kalam',cursive] text-xl sm:text-2xl leading-none">{streakCount} {streakCount === 1 ? 'Day' : 'Days'}</div>
              <div className="text-orange-700 dark:text-orange-400/80 text-xs sm:text-sm font-bold uppercase tracking-wide">Streak</div>
            </div>
          </div>
        </div>
        ...
        <section className="mb-10 sm:mb-16">
          <h2 className="text-2xl sm:text-3xl font-['Kalam',cursive] font-bold text-blue-900 dark:text-blue-300 mb-4 sm:mb-6 flex items-center gap-2.5 sm:gap-3"><Play className="w-6 h-6 sm:w-8 sm:h-8 fill-blue-500 text-blue-500 shrink-0" /> Pick up where you left off</h2>
          <div className="bg-blue-100 dark:bg-blue-900/40 p-4 sm:p-8 rounded-2xl sm:rounded-3xl border-3 sm:border-4 border-blue-400 dark:border-blue-700/50 shadow-[4px_4px_0px_0px_rgba(96,165,250,1)] sm:shadow-[8px_8px_0px_0px_rgba(96,165,250,1)] dark:shadow-[4px_4px_0px_0px_rgba(30,58,138,1)] sm:dark:shadow-[8px_8px_0px_0px_rgba(30,58,138,1)] transform -rotate-0.5 sm:-rotate-1 relative">
            ...
            {isLoading ? (
              <div className="flex items-center gap-3 py-4 sm:py-5 font-bold text-sm sm:text-base text-blue-700 dark:text-blue-300"><LoaderCircle className="w-6 h-6 sm:w-7 sm:h-7 animate-spin" /> Loading your course…</div>
            ) : continueCourse ? (
              <div className="flex flex-col md:flex-row justify-between gap-6 sm:gap-8 items-start md:items-center">
                <div className="flex-grow w-full">
                  ...
                  <div className="text-blue-700 dark:text-blue-400 font-bold text-xs sm:text-sm">{clampProgress(continueCourse.progressPercentage)}% Completed</div>
                </div>
                <button onClick={() => navigate(`/courses/${continueCourse.id}`)} className="bg-blue-500 dark:bg-blue-600 hover:bg-blue-600 dark:hover:bg-blue-500 text-white font-bold py-3.5 sm:py-4 px-6 sm:px-8 rounded-xl shadow-[0_4px_0px_0px_rgba(29,78,216,1)] sm:shadow-[0_8px_0px_0px_rgba(29,78,216,1)] dark:shadow-[0_4px_0px_0px_rgba(30,58,138,1)] sm:dark:shadow-[0_8px_0px_0px_rgba(30,58,138,1)] transform transition hover:translate-y-1 hover:shadow-[0_2px_0px_0px_rgba(29,78,216,1)] dark:hover:shadow-[0_2px_0px_0px_rgba(30,58,138,1)] shrink-0 w-full md:w-auto text-lg sm:text-xl font-['Kalam',cursive] border-2 border-blue-700 dark:border-blue-800">Continue Learning</button>
              </div>
            ) : (
              <div className="py-4 sm:py-5">
                <h3 className="text-xl sm:text-3xl font-bold font-['Kalam',cursive] text-blue-950 dark:text-blue-200 mb-1.5 sm:mb-2">No course in progress yet</h3>
                <p className="text-blue-800 dark:text-blue-300/80 text-xs sm:text-base font-medium mb-4 sm:mb-6">Generate a new course or reuse one from the library to start learning.</p>
                <button onClick={() => openModal()} className="bg-blue-500 text-white font-bold py-2.5 sm:py-3 px-5 sm:px-6 rounded-xl font-['Kalam',cursive] text-lg sm:text-xl border-2 border-blue-700 w-full sm:w-auto">Create your first course</button>
              </div>
            )}
          </div>
        </section>
        ...
        <section>
          <h2 className="text-2xl sm:text-3xl font-['Kalam',cursive] font-bold text-gray-800 dark:text-gray-200 mb-4 sm:mb-6 flex items-center gap-2.5 sm:gap-3"><Clock className="w-6 h-6 sm:w-7 sm:h-7 text-gray-600 dark:text-gray-400 shrink-0" /> Recent Courses</h2>
          {isLoading ? (
            <div className="py-8 flex items-center gap-3 font-bold text-sm sm:text-base text-gray-600 dark:text-gray-300"><LoaderCircle className="w-5 h-5 sm:w-6 sm:h-6 animate-spin" /> Loading recent courses…</div>
              ...
              {dashboard.recentCourses.map((course, index) => {
                ...
                return (
                  <button key={course.id} onClick={() => navigate(`/courses/${course.id}`)} className={`${style.card} ${style.border} p-4 sm:p-6 rounded-2xl border-2 shadow-[3px_3px_0px_0px_rgba(100,116,139,.35)] sm:shadow-[4px_4px_0px_0px_rgba(100,116,139,.35)] ${index % 2 ? '-rotate-1' : 'rotate-1'} hover:rotate-0 transition-transform cursor-pointer relative text-left`}>
                    ...
                    <p className={`${style.text} font-bold text-xs sm:text-sm`}>{progress}% Completed</p>
                  </button>
                );
              })}
            </div>
            ...
            <div className="text-center p-4 rounded-2xl border-2 border-dashed border-gray-300 dark:border-gray-700 bg-white/70 dark:bg-gray-800/70 py-10 sm:py-14">
              ...
              <h3 className="font-['Kalam',cursive] text-xl sm:text-2xl font-bold text-gray-700 dark:text-gray-200">Your recent courses will appear here</h3>
              <p className="mt-1 sm:mt-2 text-xs sm:text-base font-semibold text-gray-500 dark:text-gray-400">Start or reuse a course to see it on your dashboard.</p>
            </div>
          )}
        </section>
      </div>
